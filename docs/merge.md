...
Ultimate Website Skills 3.0.0 is the combined plugin. All 21 Atelier section IDs, its bone/ink/cinema presets, CSS/motion assets and supporting references are retained alongside Cinematic's newer tools and fable preset.

Use `/webdesign` or `/ultimate-website-skills:atelier` in Claude Code. The CLI alias `node scripts/atelier.mjs` dispatches to the same implementation and retains the bone default. `node scripts/webdesign.mjs` is the canonical CLI. There is one design skill, so installing the combined plugin does not load competing copies.
...
[Claude Design connection and handoff](../skills/ultimate-website-skills/references/claude-design.md).
